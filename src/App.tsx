export default function App() {
  const navItems = [
    'Inicio','Competiciones','Equipos','Jugadores','Partidos','Apuestas','Inteligencia','Estadísticas'
  ];
  const kpis = [
    ...
    <div className="min-h-screen bg-[#040b14] text-gray-200 flex">
      <aside className="w-64 bg-[#0b1422] border-r border-slate-800 p-4 hidden md:flex flex-col">
        ...
        <nav className="space-y-1 text-sm">
          {navItems.map(item=>(
            <div key={item} className={`px-3 py-2 rounded hover:bg-slate-800 cursor-pointer ${item==='Inicio' ? 'bg-slate-800 text-white' : 'text-gray-300'}`}>{item}</div>
          ))}
        </nav>
      ...
      <main className="flex-1 p-6 space-y-6">
        ...
        <section className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {kpis.map(k=>(
            <div key={k.label} className="bg-[#0b1422] rounded-xl p-4 border border-slate-800">
